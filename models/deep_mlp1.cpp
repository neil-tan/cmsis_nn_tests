...
template <class T>
void print_tensor_d(S_TENSOR tensor) {
  printf("printing tensors %s: \r\n", tensor->getName().c_str());
  const T *out_buff = tensor->read<T>(0, 1);
...
void print_tensor_f(S_TENSOR tensor) {
  printf("printing tensors %s: \r\n", tensor->getName().c_str());
  const float *out_buff = tensor->read<float>(0, 1);
  for(size_t i = 0; i < tensor->getSize(); i++) {
    printf("%f ", *(out_buff + i));
  }
  printf("\r\n");
